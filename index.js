      ...
      if (url.includes('xenispa.com')) {
          ...
          $('.schedule-item').each((i, el) => {
              ...
              if (dateMatch) {
                  ...
                  const cleanTime = timeRaw.replace(/\s+/g, '').replace('〜', '～');
                  map[dateKey] = (cleanTime.includes('休み') || !/\d/.test(cleanTime)) ? { time: "-", shop: "ゼニスパ" } : { time: cleanTime, shop: "ゼニスパ" };
              }
          });
          ...
          $('#girl_sukkin li').each((i, el) => {
              ...
              if (dateMatch) {
                  ...
                  const isHoliday = rawText.includes("休") || !/\d/.test(rawText);
                  if (isHoliday) {
                      map[dateKey] = { time: "-", shop: null };
                  } else {
                      const shopMatch = rawText.match(/\((.+)\)/);
                      const timeValue = rawText.replace(/\(.+\)/, '').replace('-', '～');
                      map[dateKey] = { time: timeValue, shop: shopMatch ? shopMatch[1] : null };
                  }
              }
          });
      }
