...
// --- 3. 固定地域の割り当て (ハイフンが含まれる変数を修正) ---
const getPageData = () => {
  return {
    "北海道_☆☆☆": [cast?.kaho, cast?.kaho_2].filter(Boolean),
    "東京都_☆☆☆☆☆": [cast?.asami, cast?.koyuki, cast?.minami_shiori, cast?.meari, cast?.rumina, cast?.thuru, cast?.yuutuki_kokona].filter(Boolean), // thu-ru を thuru に修正
    "東京都_☆☆☆☆": [cast?.thukino_aina].filter(Boolean),
    "東京都_☆☆☆": [cast?.juri, cast?.momoka_hotaru].filter(Boolean),
    "東京都_★★★★★": [cast?.hanamo, cast?.sumeragi_yuzu_tokyo, cast?.sumeragi_yuzu_tokyo_2, cast?.sumeragi_yuzu_tokyo_3].filter(Boolean),
...
function getBusinessDateKey() {
  ...
  // 【重要】朝5時より前なら、日付を1日戻す（深夜営業対応）
  if (now.getHours() < 5) {
      now.setDate(now.getDate() - 1);
  }
  
  // スケジュール表のキー形式 (例: "1/26") に合わせる
...
async function getScheduleCached(url) {
  ...
  try {
      ...
      $('#girl_sukkin li').each((i, el) => {
          ...
          // 数字だけを抽出 (例: "01/26" でも "1/26" でも "1" と "26" に分解)
...
app.get("/:pageId?", async (req, res) => {
    ...
    if (!pageId || pageId === "today") {
        ...
        // フィルタリングの強化: timeが "-" 以外かつ、数字が含まれている場合のみ表示
