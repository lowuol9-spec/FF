...
const https = require('https'); // 追加：証明書エラー回避用
const app = express();
const PORT = process.env.PORT || 8000; 

// 証明書エラーを無視するaxiosインスタンスを作成
...
try {
    ...
    console.error("❌ casts.json の読み込み失敗:", err.message);
}
...
// --- 6. スケジュール取得ロジック (3サイト対応 & axiosInstance使用) ---
async function getScheduleCached(url) {
  ...
  try {
      // axios ではなく axiosInstance を使用してSSLエラーを無視
...
app.get("/:pageId?", async (req, res) => {
    ...
    function generateCastHtml(g, schedule, dateKey) {
        ...
        // --- 画像URLの修正ロジック ---
        // 1. 末尾の /150x200 などのゴミを削除
        // 2. ブラウザのセキュリティブロックを回避するために referrerpolicy="no-referrer" を付与
        let safeImg = (g.img || "").replace(/\/\d+x\d+$/, "");
        ...
        return `<li class="item">
            <figure class="thumbnail">
                <img src="${safeImg}" alt="画像" referrerpolicy="no-referrer">
            </figure>
            <dl>
