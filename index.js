...
try {
    ...
    if (fs.existsSync(castsPath)) {
        ...
        console.error("⚠️ casts.json が見つかりません。空のデータで起動します。");
    }
} catch (err) {
    console.error("❌ casts.json の読み込み/解析に失敗:", err.message);
}

// --- 3. 固定地域の割り当て (データがない場合の安全策を追加) ---
const getPageData = () => {
  return {
    "北海道_☆☆☆": [cast?.kaho, cast?.kaho_2].filter(Boolean),
    "東京都_☆☆☆☆☆": [cast?.asami, cast?.koyuki, cast?.minami_shiori, cast?.meari, cast?.rumina].filter(Boolean),
    "東京都_☆☆☆☆": [cast?.thukino_aina].filter(Boolean),
    "東京都_☆☆☆": [cast?.momoka_hotaru].filter(Boolean),
    "東京都_★★★★★": [cast?.sumeragi_yuzu_tokyo, cast?.sumeragi_yuzu_tokyo_2, cast?.sumeragi_yuzu_tokyo_3].filter(Boolean),
    "神奈川県_☆☆☆☆☆": [cast?.minami_shiori_2].filter(Boolean),
    "新潟県_☆☆☆☆☆": [cast?.minami_shiori_3].filter(Boolean),
    "新潟県_☆☆☆☆☆": [cast?.minami_shiori_4].filter(Boolean),
    "愛知県_☆☆☆☆☆": [cast?.minami_shiori_5].filter(Boolean),
...
app.get("/:pageId?", async (req, res) => {
    ...
    const rankOrder = ["☆☆☆☆☆","★★★★★","☆☆☆☆","★★★★", "☆☆☆","★★★","☆☆","★★","☆","★"];
    const pageData = getPageData(); // 最新のcastデータから生成
    ...
    if (!pageId || pageId === "today") {
        ...
        if (presentGirls.length === 0) {
            itemsHtml += `<p style="text-align:center; padding:20px;">現在、出勤データはありません。</p>`;
        } else {
