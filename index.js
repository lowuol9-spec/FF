...
const fs = require('fs'); // ファイル読み込み用に追加
const express = require("express");
const axios = require('axios');
const cheerio = require('cheerio');
const app = express();
const PORT = process.env.PORT || 8000; // Render等の公開サーバー対応

app.use(express.static(path.join(__dirname, 'public')));

// --- 2. データの読み込み (casts.json から取得) ---
let cast = {};
try {
    const rawData = fs.readFileSync(path.join(__dirname, 'casts.json'), 'utf8');
    cast = JSON.parse(rawData);
    console.log("✅ casts.json の読み込みに成功しました");
} catch (err) {
    console.error("❌ casts.json の読み込みに失敗しました:", err.message);
}

// --- 3. 固定地域の割り当て ---
// casts.json のキー名を使用して紐付けます
const pageData = {
...
app.get("/:pageId?", async (req, res) => {
    ...
    if (!pageId || pageId === "today") {
        ...
        Object.entries(pageData).forEach(([pName, members]) => {
            ...
            if (foundRank) members.forEach(m => { if(m) castToRank[m.url] = foundRank; });
        });
        ...
        const results = await Promise.all(girls.map(g => g ? getScheduleCached(g.url) : {}));
