...
// 証明書エラーを無視し、User-Agentをブラウザに偽装するaxiosインスタンス
...
// --- 1.5 画像プロキシ (カミスガ等のMixed Content対策) ---
app.get('/proxy-img', async (req, res) => {
    const targetUrl = req.query.url;
    if (!targetUrl) return res.status(400).send('No URL');
    try {
        const response = await axiosInstance.get(targetUrl, { responseType: 'arraybuffer' });
        res.set('Content-Type', response.headers['content-type']);
        res.send(response.data);
    } catch (e) {
        // 画像取得に失敗した場合はカミスガのスペーサー画像を返す
        res.redirect('http://www.kami-sugamo.com/asset/img/spacer300x450.png');
    }
});
...
try {
    ...
    if (fs.existsSync(castsPath)) {
        cast = JSON.parse(fs.readFileSync(castsPath, 'utf8'));
        console.log("✅ casts.json 読み込み完了");
    }
} catch (err) { console.error("❌ casts.json 読み込み失敗:", err.message); }

// --- 3. 固定地域の割り当て ---
const getPageData = () => {
  return {
    "北海道_☆☆☆☆": [cast?.yui_ami, cast?.minami].filter(Boolean),
    "北海道_☆☆☆": [cast?.kaho, cast?.kaho_2].filter(Boolean),
    "宮城県_☆☆☆☆": [cast?.hanabi].filter(Boolean),

    "東京都_☆☆☆☆☆": [cast?.asami, cast?.bella, cast?.furansowa, cast?.himena, cast?.hinano_yuna, cast?.koyuki, cast?.minami_shiori, cast?.meari, cast?.rumina, cast?.rico, cast?.thuru, cast?.kanae_rina, cast?.yuutuki_kokona, cast?.kobayashi_ruru, cast?.komugi, cast?.lora, cast?.rurutan, cast?.seisyou_nagon, cast?.thiho].filter(Boolean), 
    "東京都_☆☆☆☆": [cast?.kagura_run, cast?.midori, cast?.junna, cast?.rukachi, cast?.rukachi_2, cast?.rukachi_3, cast?.thukino_aina].filter(Boolean),
    "東京都_☆☆☆": [cast?.juri, cast?.momoka_hotaru,cast?.hashimoto_airi].filter(Boolean),
    "東京都_★★★★★": [cast?.amaharu_noa, cast?.hanamo, cast?.sayuu_nanaha, cast?.sayuu_nanaha_2, cast?.sumeragi_yuzu_tokyo, cast?.sumeragi_yuzu_tokyo_2, cast?.sumeragi_yuzu_tokyo_3, cast?.tojo_ibuki].filter(Boolean),
    "東京都_★★★★": [cast?.vivian].filter(Boolean),
    "東京都_★★★": [cast?.aoyama_kia].filter(Boolean),

    "神奈川県_☆☆☆☆☆": [cast?.minami_shiori_2, cast?.noeru, cast?.renon].filter(Boolean),
    "千葉県_☆☆☆☆☆": [cast?.aika_yura].filter(Boolean),
    "千葉県_☆☆☆☆": [cast?.erika].filter(Boolean),
    "茨城県_☆☆☆": [cast?.hashimoto_airi_2].filter(Boolean),
    "埼玉県_★★★★★": [cast?.tojo_ibuki_2].filter(Boolean),
    "群馬県_☆☆☆☆": [cast?.you].filter(Boolean),
    "栃木県_☆☆☆☆": [cast?.mana].filter(Boolean),
    "新潟県_☆☆☆☆☆": [cast?.minami_shiori_3, cast?.minami_shiori_4].filter(Boolean),
    "新潟県_☆☆☆☆": [cast?.nagisa, cast?.shimazaki_niina].filter(Boolean),
    "長野県_☆☆☆☆☆": [cast?.minami_shiori_4].filter(Boolean),
    "愛知県_☆☆☆☆☆": [cast?.minami_shiori_5, cast?.riri, cast?.rito, cast?.erena_2].filter(Boolean),
    "愛知県_☆☆☆☆": [cast?.erena].filter(Boolean),
    "静岡県_☆☆☆☆": [cast?.mithiru].filter(Boolean),
    "静岡県_☆☆☆": [cast?.hashimoto_airi_3].filter(Boolean),
    "岐阜県_☆☆☆☆": [cast?.pan].filter(Boolean),
    "大阪府_☆☆☆☆": [cast?.nansei_seira, cast?.karen, cast?.karen_2].filter(Boolean),
    "大阪府_★★★★★": [cast?.sumeragi_yuzu_tokyo_4].filter(Boolean),
    "滋賀県_☆☆☆☆": [cast?.mei].filter(Boolean),
    "京都府_☆☆☆☆☆": [cast?.sakurako].filter(Boolean),
    "京都府_☆☆☆": [cast?.naho].filter(Boolean),
    "岡山県_☆☆☆☆☆": [cast?.minami_shiori_6].filter(Boolean),
    "高知県_★★★★★": [cast?.tojo_ibuki_3].filter(Boolean),
    "福岡県_☆☆☆☆": [cast?.ramu_fukuoka].filter(Boolean),
    "福岡県_☆☆☆": [cast?.yumi].filter(Boolean),
    "福岡県_★★★★★": [cast?.sumeragi_yuzu_tokyo_5].filter(Boolean),
    "熊本県_☆☆☆☆☆": [cast?.oshino_otone, cast?.oshino_otone_2].filter(Boolean),
    "鹿児島県_★★★★★": [cast?.tojo_ibuki_4].filter(Boolean)
  };
};
...
// --- 6. スケジュール取得ロジック (ゼニスパのセレクタをより広範囲に修正) ---
async function getScheduleCached(url) {
  if (!url || url === "#") return {};
  const now = Date.now();
  if (scheduleCache[url] && (now - scheduleCache[url].time < CACHE_TIMEOUT)) return scheduleCache[url].data;
  ...
  try {
      ...
      if (url.includes('xenispa.com')) {
          // --- ゼニスパ：テーブル構造 (tbl-calendar) に対応 ---
          $('table.tbl-calendar tr').each((i, el) => {
              const dateRaw = $(el).find('td').first().text().trim(); // "1/26 月"
              const contentTd = $(el).find('td').eq(1); // スケジュール本文
              
              // 日付の正規表現を修正 (曜日があっても数字だけ抜く)
              const dateMatch = dateRaw.match(/(\d{1,2})\/(\d{1,2})/);
              if (dateMatch) {
                  const dateKey = `${parseInt(dateMatch[1])}/${parseInt(dateMatch[2])}`;
                  
                  // 時間と店舗名を抽出
                  let fullText = contentTd.text().trim().replace(/\s+/g, ' ');
                  
                  if (!fullText || fullText === "" || fullText.includes('休み')) {
                      map[dateKey] = { time: "-", shop: "ゼニスパ" };
                  } else {
                      // "東武練馬ルーム 12:00〜20:00" のような文字列から時間を抽出
                      const timeMatch = fullText.match(/(\d{1,2}:\d{2}\s*〜\s*\d{1,2}:\d{2})/);
                      const timeStr = timeMatch ? timeMatch[1].replace('〜', '～') : fullText;
                      
                      // ルーム名があれば抽出、なければデフォルト
                      const roomMatch = fullText.match(/(.*ルーム|.*店)/);
                      const shopName = roomMatch ? roomMatch[1].trim() : "ゼニスパ";
                      
                      map[dateKey] = { time: timeStr, shop: shopName };
                  }
              }
          });
          ...
          $('#girl_sukkin li').each((i, el) => {
              ...
              if (dateMatch) {
                  ...
                  if (rawText.includes("休") || !/\d/.test(rawText)) {
                      map[dateKey] = { time: "-", shop: null };
                  } else {
                      const shopMatch = rawText.match(/\((.+)\)/);
                      map[dateKey] = { time: rawText.replace(/\(.+\)/, '').replace('-', '～'), shop: shopMatch ? shopMatch[1] : null };
                  }
              }
          });
      }
...
app.get("/:pageId?", async (req, res) => {
    ...
    function generateCastHtml(g, schedule, dateKey) {
        ...
        // --- 画像URL処理 (プロキシ経由に変更) ---
        let rawImg = (g.img || "").replace(/\/\d+x\d+$/, "");
        let safeImg = rawImg;
        // カミスガ等の http 画像は自サーバーのプロキシを通す
        if (rawImg.startsWith('http://www.kami-sugamo.com')) {
            safeImg = `/proxy-img?url=${encodeURIComponent(rawImg)}`;
        }
        ...
        return `<li class="item">
            <figure class="thumbnail">
                <img src="${safeImg}" alt="画像" referrerpolicy="no-referrer" onerror="this.src='http://www.kami-sugamo.com/asset/img/spacer300x450.png'">
            </figure>
            <dl>
